...
# Add src to path so we can import keyplay
sys.path.insert(0, os.path.join(os.path.dirname(__file__), '..', 'src'))

from keyplay import RiskeycapBase, KEYCAPS, make_commands


# Mock args class for testing the make_commands function
class MockArgs:
...
class TestMakeCommandsFunction(unittest.TestCase):
    ...
    def test_make_commands_specific_keycap_names(self):
        ...
        if len(KEYCAPS) > 0:
            ...
            # Check that we got commands
            self.assertGreater(len(commands), 0, f"Should generate commands for keycap '{test_keycap_name}'")
    ...
    def test_make_commands_with_legends_flag(self):
        ...
        if test_keycap:
            ...
            try:
                ...
                self.assertGreater(len(commands), 1, "Should generate commands for both keycap and legends when legends=True")
                ...
                self.assertIsNotNone(keycap_command, "Should have a command for the keycap")
                self.assertIsNotNone(legends_command, "Should have a command for the legends")
            finally:
    ...
    def test_make_commands_with_force_flag(self):
        ...
        if len(KEYCAPS) > 0:
            ...
            self.assertGreater(len(commands), 0, "Should process keycap when force=True")
    ...
    def test_make_commands_skips_existing_files_when_force_false(self, mock_exists):
        ...
        if len(KEYCAPS) > 0:
            ...
            self.assertEqual(len(commands), 0, "Should not generate commands when files exist and force=False")
    ...
    def test_make_commands_with_empty_names_list(self):
        ...
        self.assertGreater(len(commands), 0, "Should process all keycaps when names list is empty")
    ...
    def test_make_commands_with_nonexistent_keycap_name(self):
        ...
        self.assertEqual(len(commands), 0, "Should not generate commands when keycap name is not found")
