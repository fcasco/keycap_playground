...
KEYCAPS = [
    ...
    riskeycap_1_25U(name="MoreFun", legends=["More", "Fun"], trans=[[3, 2.5, 0], [3, -2.5, 0]],
                ...
                if keycap.name.lower() == name.lower():
                    ...
                    if not args.force:
                        if os.path.exists(f"{args.out}/{keycap.name}.{keycap.file_type}"):
                            print(f"{args.out}/{keycap.name}.{keycap.file_type} exists; "
                                  f"skipping...")
                            exists = True
                    if not exists:
            ...
            for legend in KEYCAPS:
                ...
                if not args.force:
                    if os.path.exists(f"{args.out}/{legend.name}.{legend.file_type}"):
                        print(f"{args.out}/{legend.name}.{legend.file_type} exists; "
                              f"skipping...")
                        continue
